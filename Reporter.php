...
class Reporter{
	...
	protected function isIgnore($srcPath){
		var_dump('没进来？');
		var_dump($this->ignoreFile);
		$ignores = file_get_contents($this->ignoreFile);
		$count = mb_strlen($ignors,'utf-8');
		var_dump('统计的字数',$count);
		
		
		foreach ($ignores as $key => $value) {			
			$count = substr_count($srcPath,$value);
			//var_dump('统计的数量为',$count);
			if($count > 0){
				return true;
			}
		}
...
wo 说我的牛牛
	...
	protected function mergeCoverages(){
		...
		//var_dump('怀疑是这里的问题',$coverageFiles);
